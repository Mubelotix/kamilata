...
pub(crate) async fn init_routing<const N: usize, D: Document<N>>(db: Arc<Db<N, D>>, our_peer_id: PeerId, remote_peer_id: PeerId) -> HandlerTaskOutput {
    ...
    match (pending_out_routing_task, pending_in_routing_task) {
        (Some(t1), Some(t2)) => HandlerTaskOutput::Many(vec![
            HandlerTaskOutput::NewPendingTask { tid: Some(1), pending_task: t1},
            HandlerTaskOutput::NewPendingTask { tid: Some(2), pending_task: t2},
        ]),
        (Some(t1), None) => HandlerTaskOutput::NewPendingTask { tid: Some(1), pending_task: t1},
