...
pub(crate) async fn init_routing<const N: usize, D: Document<N>>(db: Arc<Db<N, D>>, our_peer_id: PeerId, remote_peer_id: PeerId) -> HandlerTaskOutput {
    ...
    if matches!(in_routing_state, UnderMin | UnderTarget) {
        pending_in_routing_task = Some(pending_post_filters(Arc::clone(&db), our_peer_id, remote_peer_id));
    }

    let out_routing_state = config.out_routing_peers.state(db.out_routing_peers().await);
    let mut pending_out_routing_task = None;
    if matches!(out_routing_state, UnderMin | UnderTarget) {
        pending_out_routing_task = Some(pending_post_filters(Arc::clone(&db), our_peer_id, remote_peer_id));
    }
